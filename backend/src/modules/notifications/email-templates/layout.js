...
const {
  ...
  resolveThemeColors,
  normalizeHexColor
} = require("../../../shared/constants/default-branding");

const BRAND = ELVA_DEFAULT_BRANDING.primaryColor;
const BRAND_LIGHT = "#243a56";
const ACCENT = ELVA_DEFAULT_BRANDING.secondaryColor;
const BG = "#f4f6f9";
const TEXT = "#1e293b";
const MUTED = "#64748b";
...
const resolveEmailBrand = (branding = null) => {
  const colors = resolveThemeColors({
    primaryColor: branding?.primaryColor,
    secondaryColor: branding?.secondaryColor
  });
  const supportDisplayName =
    (typeof branding?.supportDisplayName === "string" && branding.supportDisplayName.trim()) ||
    ELVA_DEFAULT_BRANDING.supportDisplayName;
  const organizationName =
    (typeof branding?.organizationName === "string" && branding.organizationName.trim()) ||
    branding?.tenantName ||
    ELVA_DEFAULT_BRANDING.organizationName;

  let logoSrc = getLogoUrl();
  if (typeof branding?.logoUrl === "string" && branding.logoUrl.trim()) {
    const candidate = branding.logoUrl.trim();
    // Only allow http(s) absolute URLs — block javascript: / data: injection
 ...
 * Tenant branding is best-effort; failures never block rendering.
 */
const renderEmailLayout = ({
  ...
  let brand;
  try {
    brand = resolveEmailBrand(branding);
  } catch {
    brand = resolveEmailBrand(null);
  }

  const supportDisplayName = brand.supportDisplayName;
  ...
  const defaultFooter = `
    <p style="margin:0 0 8px;font-size:13px;color:${MUTED};">
      Need help? Email <a href="mailto:${escapeHtml(getSupportEmail())}" style="color:${brand.secondary};text-decoration:none;">${escapeHtml(getSupportEmail())}</a>
    </p>
    <p style="margin:0;font-size:12px;color:#94a3b8;">© ${new Date().getFullYear()} ${escapeHtml(brand.organizationName)}</p>
  `;

  const heroGradient = `linear-gradient(135deg,${brand.primary} 0%,${brand.primaryHover} 45%,${brand.secondary} 100%)`;
    ...
    <tr>
      <td align="center" style="padding:32px 16px;">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(19,41,75,0.08);">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td style="vertical-align:middle;">
                    <img src="${escapeHtml(brand.logoSrc)}" alt="${escapeHtml(supportDisplayName)}" width="48" height="48" style="display:inline-block;vertical-align:middle;border-radius:8px;" />
                    <span style="display:inline-block;vertical-align:middle;margin-left:12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${brand.secondary};">${escapeHtml(supportDisplayName)}</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:0;background:${heroGradient};">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
...
const resolveButtonColor = (brandColor) =>
  normalizeHexColor(brandColor) || BRAND;

const renderParagraph = (text) =>
  `<p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${TEXT};">${text}</p>`;

const renderCtaButton = (label, href, brandColor = BRAND) => {
  const color = resolveButtonColor(brandColor);
...
const renderCodeBlock = (code, label = "Your verification code", brandColor = BRAND) => {
  const color = resolveButtonColor(brandColor);
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:20px 0;">
    <tr>
      <td align="center" style="padding:24px;background-color:#f1f5f9;border-radius:10px;border:1px solid #e2e8f0;">
        ...
        <p style="margin:0;font-size:32px;font-weight:700;letter-spacing:0.2em;color:${color};font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;">${escapeHtml(code)}</p>
      </td>
    </tr>
  </table>`;
};

const renderInfoBox = (html, brandColor = ACCENT) => {
  const accent = resolveButtonColor(brandColor) || ACCENT;
