...
const { stripClientTenantId } = require("../../shared/utils/tenant-scope.util");
...
router.post(
  ...
  validate,
  asyncHandler(async (req, res) => {
    const payload = stripClientTenantId(req.body);
    const data = await omnichannelEngine.processInbound({
      ...payload,
      tenantId: req.tenant._id,
      source: payload.source || CONVERSATION_SOURCES.API
    });
    res.json({ data });
  })
);
