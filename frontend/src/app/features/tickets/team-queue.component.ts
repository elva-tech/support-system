...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <div>
        <h2 class="text-2xl font-bold text-slate-900">Team Queue</h2>
        <p class="text-sm text-slate-500">
          @if (auth.isAdmin()) {
            All team tickets (filter by team)
          } @else {
...
export class TeamQueueComponent implements OnInit {
  ...
  private load(): void {
    ...
    this.api.listTeam(params).subscribe({
      next: (res) => {
        this.tickets.set(res.data);
        this.pagination.set(res.pagination);
      },
      error: (err: HttpErrorResponse) => this.error.set(err.error?.message || 'Failed to load team queue')
    });
  }
}
